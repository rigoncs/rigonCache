...
namespace RigonCache {
    ...
    // 前向声明
    template<typename Key, typename Value> class RLruCache;
    ...
    public:
        ...
        friend class RLruCache<Key, Value>;
    };

    template<typename Key, typename Value>
    class RLruCache : public RICachePolicy<Key, Value> {
    public:
        using LruNodeType = LruNode<Key, Value>;
        using NodePtr = std::shared_ptr<LruNodeType>;
        using NodeMap = std::unordered_map<Key, NodePtr>;

        RLruCache(int capacity)
            : capacity_(capacity) {
            initializeList();
        }
    ...
    public:
        RLruKCache(int capacity, int historyCapacity, int k)
            ...
            , historyList_(std::make_unique<RLruCache<Key, size_t>>(historyCapacity))
            , k_(k)
        {}
    ...
    private:
        int k_; // 进入缓存队列的评判标准
        std::unique_ptr<RLruCache<Key, size_t>> historyList_; // 访问数据历史记录(value为访问次数)
        std::unordered_map<Key, Value> historyValueMap_; // 存储未达到k次访问的数据值
    };

    // LRU优化：对LRU进行分片
    template<typename Key, typename Value>
    class RHashLruCaches {
    public:
        RHashLruCaches(size_t capacity, int sliceNum)
            : capacity_(capacity)
        , sliceNum_(sliceNum > 0 ? sliceNum : std::thread::hardware_concurrency()) {
            size_t sliceSize = std::ceil(capacity / static_cast<double>(sliceNum_)); // 获取每个分片的大小
            for (int i = 0; i < sliceNum_; ++i) {
                lruSliceCaches_.emplace_back(new RLruCache<Key, Value>(sliceSize));
            }
        }
        void put(Key key, Value value) {
